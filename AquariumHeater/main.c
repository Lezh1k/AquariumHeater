...
register uint8_t SINTERRUPTS asm("r4"); //todo use another register.
register uint8_t DST_T asm("r5");
register uint8_t CUR_T asm("r6");
register uint8_t CUR_LT asm("r7");
...
main(void) {  
  ...
  max7219_init();
	...
	enable_pcie_int();
	//DDRB |= PORT_BTN_UP | PORT_BTN_DOWN;
...
handle_sint_adc() {
	...
	do {
		...
		/*{
			static int8_t ind0 = 0;
			ind0 = ~ind0;
			if (ind0) PORTB |= PORT_BTN_DOWN;
			else PORTB &= ~PORT_BTN_DOWN;
		}*/
		...
		st = TERMISTOR_RREF / (1024.0f / adc_avg - 1);
		st /= TERMISTOR_R0;
		st = logf(st);
		st /= TERMISTOR_B;
